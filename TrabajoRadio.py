# Realizado por: Andres Molina, Sebastian Ortiz y Julian Mosquera
# Asesor:        German David Goes Sanchez
# Materia:       Radiocomunicaciones - ITM
#                Medellin -- Marzo-2022
...
from math import *
from tkinter import ttk,Text,Label,Button,Entry,Frame,Tk,messagebox,PhotoImage, Image
...
def creditos():
   filewin = Toplevel(root)
   filewin.geometry("390x100")
   filewin.configure(background='white')
   lblcreditos= ttk.Label(filewin, text= "Programado por Andrés Molina, Sebastian Ortiz y Julian Mosquera \n" + 
                                "\nCon la asesoria del Profesor del ITM: Germán David Goez Sánchez \n" + 
                                "\nPara la materia Radiocomunicaciones. ", background="white", font='arial 9 bold').place(x=0, y=0)
...
filemenu.add_command(label="SALIR", command=root.destroy, font='arial 10 bold')
...
menubar.add_cascade(label="Importante", menu=helpmenu)
...
def Zonafres():
    ...
    lblResultados= ttk.Label(p1, text=" La zona de Fresnel es =  " + str(resultado) + " Metros").place(x=50, y=170)
...
def ZonafresN():
    ...
    lblResultados= ttk.Label(p1, text=" La zona n de Fresnel es =   "  + str(Fn)+ " Metros").place(x=50, y=390)
...
def Okuha():
    ...
    lblResultados= ttk.Label(p3, text=" La pérdida básica de propagación en ciudad media-pequeña es =   " + str(Lb)+ " dB").place(x=50, y=390)
...
def Calculo ():
        ...
        lblResultados= ttk.Label(p2, text=" Los Calculos del enlace son: \n"
                                            ...
                                            + "\n La Potencia recibida (Friis) es =   " + str(fr)+" dBm").place(x=50, y=330)   # Muestra los resultados como etiquetas en un lugar de la pantalla
...
botoncalculo= ttk.Button(p2, text="Calcular Enlace", command= Calculo).place(x=350,y=300)
...
def COST ():
    lost= 42.6+(26*log10(d4.get()))+(20*log10(f4.get()))
...
Entry(p4, textvariable= f4).place(x=300,y=250)
Entry(p4, textvariable= d4).place(x=300,y=270)
...
#=========Solo mostrar mapa de colombia con un marcador ==================================================================
# create map widget
...
map_widget.set_address("Colombia Cerro el Volador", marker=True)
